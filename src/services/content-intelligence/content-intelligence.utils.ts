...
export function inferContentIntelligence(items: CollectedContentItem[]): ContentIntelligenceReport[] {
  return items.map((item, idx) => {
    ...
    const isHighReach = item.views > 80_000 || item.isPinned;

    const hookStrength = Math.min(98, Math.max(78, isHighReach ? 92 + (seed % 7) : 82 + (seed % 10)));
    const viralityScore = Math.min(99, Math.max(72, Math.round((item.likes + item.comments * 4) / (item.views || 1) * 650 + (isHighReach ? 25 : 10))));

    const estSaveRate = Number((Math.min(8.5, Math.max(2.1, (item.likes / Math.max(1, item.views)) * 35 + 1.8))).toFixed(1));
    const estShareRate = Number((Math.min(6.2, Math.max(1.4, (item.comments / Math.max(1, item.views)) * 120 + 1.2))).toFixed(1));
    ...
    return {
      ...
      virality: {
        viralityScore,
        successProbability: viralityScore >= 88 ? "Very High (88%+ Top Tier Algorithm Push)" : viralityScore >= 80 ? "High (Consistent Above-Average Reach)" : "Moderate (Solid Core Audience Engagement)",
        confidence: 94,
      },
      winningFactors: [
