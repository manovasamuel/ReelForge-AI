...
import { Activity, Cpu, ShieldCheck, AlertTriangle, CheckCircle2, RefreshCw, Zap, BarChart3, Calendar, DollarSign, AlertCircle, Infinity as InfinityIcon } from "lucide-react";
...
export function AiTelemetryPanel() {
  ...
  if (loading) {
    return (
      <div className="p-6 bg-card border border-border rounded-md space-y-6">
    ...
    return (
      <div className="p-6 bg-rose-950/20 border border-rose-500/30 rounded-md flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <button
          ...
          disabled={refreshing}
          className="px-4 py-2 bg-muted hover:bg-muted text-white font-semibold text-sm rounded-md border border-border flex items-center gap-2 transition-all shrink-0"
        >
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="p-6 bg-muted from-gray-900/90 via-gray-900/60 border border-border rounded-md shadow-none space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            onClick={() => fetchTelemetry(true)}
            disabled={refreshing}
            className="px-4 py-2 bg-card hover:bg-muted text-muted-foreground hover:text-white font-semibold text-sm rounded-md border border-border/80 flex items-center gap-2 transition-all self-start md:self-auto shrink-0"
          >
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-card border border-border/80 rounded-md space-y-2">
            ...
            <div className="p-4 bg-card border border-border/80 rounded-md space-y-3">
              <div className="flex items-center justify-between text-xs text-muted-foreground font-medium">
            ...
            {/* Cost Estimate Card */}
            <div className="p-4 bg-card border border-border/80 rounded-md space-y-2 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between text-xs text-muted-foreground font-medium">
                ...
                <div className="text-2xl font-black text-white tracking-tight mt-1">
                  ${p.totalEstimatedCostUsd} <span className="text-xs font-semibold text-muted-foreground">USD</span>
                </div>
              </div>
              <div className="text-xs text-muted-foreground pt-2 border-t border-border/60 flex items-center gap-1.5">
              ...
              <div key={provider.providerId} className="p-4 bg-card border border-border/80 rounded-md space-y-3">
                ...
                <div className="space-y-1.5 text-xs text-muted-foreground border-t border-border/60 pt-2.5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`font-mono font-bold ${provider.consecutiveFailures > 0 ? "text-amber-400" : "text-muted-foreground"}`}>
