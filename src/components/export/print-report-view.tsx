...
export function PrintReportView({ project }: PrintReportViewProps) {
  ...
  return (
    <div className="hidden print:block bg-card text-foreground text-black p-8 font-sans max-w-4xl mx-auto space-y-12">
      {/* 1. Cover Page */}
      <section className="min-h-[85vh] flex flex-col justify-between border-b-2 border-black pb-12 break-after-page">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded border border-black bg-muted text-xs font-bold uppercase tracking-widest mb-8">
      ...
      <section className="space-y-4 break-after-page">
        ...
        <div className="grid grid-cols-2 gap-4 bg-muted p-6 rounded border border-border">
          <div>
            <div className="text-xs text-muted-foreground uppercase">Target Handle</div>
            <div className="text-lg font-bold">@{state.profile?.username || "N/A"}</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground uppercase">Industry & Niche</div>
            <div className="text-lg font-bold">{state.brandReport?.industry || "Unclassified"}</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground uppercase">Competitors Tracked</div>
            <div className="text-lg font-bold">{state.competitors?.length || 0} Accounts</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground uppercase">Content DNA Score</div>
      ...
      <section className="space-y-6 break-after-page">
        <div>
          <h2 className="text-xl font-bold border-b border-border pb-2 flex items-center gap-2 mb-3">
          ...
          ) : <p className="text-sm text-muted-foreground">No raw profile ingested.</p>}
        </div>
        ...
        <div>
          ...
          ) : <p className="text-sm text-muted-foreground">No brand analysis generated.</p>}
        </div>
      </section>
      ...
      <section className="space-y-6 break-after-page">
        ...
        ) : <p className="text-sm text-muted-foreground">No competitors recorded.</p>}

        {state.competitorAnalysis && (
          <div className="bg-muted p-4 border rounded text-sm space-y-2 mt-4">
      ...
      <section className="space-y-4 break-after-page">
        ...
        {state.contentDNA ? (
          <div className="space-y-4 text-sm">
            <div className="bg-muted p-4 rounded font-mono font-bold text-center border">
        ...
        ) : <p className="text-sm text-muted-foreground">No Content DNA blueprint available.</p>}
      </section>
      ...
      <section className="space-y-6">
        <div>
          ...
          {state.scriptPackage ? (
            <div className="space-y-4 text-sm">
              <blockquote className="bg-muted p-3 border-l-4 border-black font-semibold italic">
          ...
          ) : <p className="text-sm text-muted-foreground">No studio script generated.</p>}
        </div>
          ...
          {state.repurposePackage ? (
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className="whitespace-pre-line bg-muted p-2 border rounded mt-1">{state.repurposePackage.linkedIn.longFormPost}</p>
              </div>
              <div>
                <strong>YouTube Shorts Title:</strong> {state.repurposePackage.youtubeShorts.title}
              </div>
            </div>
          ) : <p className="text-sm text-muted-foreground">No repurpose package recorded.</p>}
        </div>
      </section>

      {/* Footer */}
      <footer className="pt-12 border-t border-border text-xs text-muted-foreground flex justify-between items-center">
